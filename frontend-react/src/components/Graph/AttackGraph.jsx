...
import { 
  ...
  User, FileText, Mail, Terminal, Sparkles, Wifi, WifiOff
} from 'lucide-react';
import { caseService } from '../../services/cases';
import { threatIntelCaseService } from '../../services/threatIntelCaseService';
import { llmService } from '../../services/llmService';
import { useGraphWebSocket, CONNECTION_STATE } from '../../hooks/useGraphWebSocket';
...
export default function AttackGraph({ caseId, data }) {
  ...
  const handleNodeAdded = useCallback((nodeData) => {
    console.log('🔵 New node via WebSocket:', nodeData.id);
  }, []);

  const handleEdgeAdded = useCallback((edgeData) => {
    console.log('🔗 New edge via WebSocket:', edgeData);
  }, []);

  const handleAnalysisComplete = useCallback((result) => {
    console.log('✅ Analysis complete via WebSocket');
  ...
  // Aplicar actualizaciones pendientes del WebSocket al grafo
  useEffect(() => {
    if (!cyRef.current) return;
    if (pendingUpdates.nodes.length === 0 && pendingUpdates.edges.length === 0) return;

    const cy = cyRef.current;
    
    // Procesar nodos
    pendingUpdates.nodes.forEach(update => {
      if (update.action === 'add') {
        const nodeData = update.data;
        // Evitar duplicados
        if (!cy.getElementById(nodeData.id).length) {
          cy.add({ data: { ...nodeData, ...nodeData.data } });
        }
      } else if (update.action === 'update') {
        const node = cy.getElementById(update.data.id);
        if (node.length) {
    ...
    // Procesar edges
    pendingUpdates.edges.forEach(update => {
      if (update.action === 'add') {
        const edgeData = update.data;
        const edgeId = edgeData.id || `e-${edgeData.source}-${edgeData.target}`;
        if (!cy.getElementById(edgeId).length) {
          cy.add({ data: { id: edgeId, ...edgeData } });
        }
      }
    });

    // Re-layout si hay nuevos elementos
    if (pendingUpdates.nodes.some(u => u.action === 'add')) {
      cy.layout({
        name: layout,
        animate: true,
    ...
    setGraphData(prev => {
      ...
      pendingUpdates.nodes.forEach(update => {
        if (update.action === 'add') {
          if (!newNodes.find(n => n.id === update.data.id)) {
            newNodes.push(update.data);
          }
        }
      });
    ...
    clearPendingUpdates();
  }, [pendingUpdates, clearPendingUpdates, layout]);
        ...
        <div className="absolute top-4 left-1/2 -translate-x-1/2 z-20">
          <div className={`flex items-center gap-2 px-3 py-1.5 rounded-full text-xs font-medium transition-all ${
            ...
            {isConnected ? (
              ...
              <><Wifi className="w-3 h-3 animate-pulse" /> Reconectando...</>
            ) : (
